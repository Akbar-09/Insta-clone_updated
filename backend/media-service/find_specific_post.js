...
async function findPost() {
    try {
        ...
        const res = await client.query(`SELECT id, "mediaUrl", "thumbnailUrl", "userId" FROM "Posts" WHERE "mediaUrl" LIKE $1 OR "thumbnailUrl" LIKE $1`, [`%${search}%`]);

        if (res.rows.length === 0) {
            console.log('No posts found with media link containing:', search);
        } else {
